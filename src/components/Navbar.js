...
const Navbar = ({ activeSection, onNavigate, onRefresh }) => {
  const [showAuth, setShowAuth] = useState(false); 
  const [isLogin, setIsLogin] = useState(true);   

  const styles = {
    header: { height: '110px', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'fixed', top: 0, width: '100%', zIndex: 1000, background: 'rgba(0,0,0,0.2)', backdropFilter: 'blur(10px)' },
    container: { width: '98%', maxWidth: '1600px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 40px' },
    logo: { fontSize: '48px', fontWeight: '900', color: 'white', display: 'flex', alignItems: 'center', gap: '15px', textDecoration: 'none', cursor: 'pointer' },
    ...
    link: (isActive) => ({ color: isActive ? '#10b981' : 'white', textDecoration: 'none', fontWeight: '700', fontSize: '22px', cursor: 'pointer', position: 'relative' }),
    ...
    // === MODAL FIX: LUÔN CANH GIỮA MÀN HÌNH (VIEWPORT) ===
    overlay: { 
      position: 'fixed', 
      top: 0, 
      left: 0, 
      width: '100vw', // Đảm bảo phủ hết chiều ngang
      height: '100vh', // Đảm bảo phủ hết chiều dọc màn hình
      backgroundColor: 'rgba(0,0,0,0.85)', 
      zIndex: 9999, // Ưu tiên cao nhất để không bị đè
      display: 'flex', 
    ...
    modal: { 
      ...
      width: '750px', 
      maxWidth: '90%', // Không cho rộng quá màn hình điện thoại
      maxHeight: '85vh', // Chống dính mép trên, tự động tạo thanh cuộn nếu quá dài
      overflowY: 'auto', // Hiện thanh cuộn nếu nội dung vượt quá 85% chiều cao
      borderRadius: '50px', 
      padding: '60px 70px', 
      position: 'relative', 
      boxShadow: '0 40px 100px rgba(0,0,0,0.5)',
      animation: 'modalFadeIn 0.3s ease-out' // Hiệu ứng hiện ra mượt
    },
    closeBtn: { position: 'absolute', top: '35px', right: '40px', background: 'none', border: 'none', fontSize: '45px', color: '#9ca3af', cursor: 'pointer' },
    ...
    <header style={styles.header}>
      <div style={styles.container}>
        ...
        <nav style={styles.nav}>
          <div style={styles.link(activeSection === 'home')} onClick={() => onNavigate('hero-section')}>
            Trang chủ
            {activeSection === 'home' && <div style={styles.underline} />}
          </div>
          <div style={styles.link(activeSection === 'schedule')} onClick={() => onNavigate('itinerary-section')}>
            Lịch trình
            {activeSection === 'schedule' && <div style={styles.underline} />}
          </div>
      ...
      {showAuth && (
        <div style={styles.overlay} onClick={() => setShowAuth(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.inputGroup}><div style={styles.inputIcon}><FontAwesomeIcon icon={faUser} /></div><input type="text" placeholder="Tên tài khoản hoặc Email" style={styles.input} /></div>
            <div style={styles.inputGroup}><div style={styles.inputIcon}><FontAwesomeIcon icon={faLock} /></div><input type="password" placeholder="Mật khẩu" style={styles.input} /></div>
            {!isLogin && (<div style={styles.inputGroup}><div style={styles.inputIcon}><FontAwesomeIcon icon={faLock} /></div><input type="password" placeholder="Xác nhận mật khẩu" style={styles.input} /></div>)}
            <button style={styles.submitBtn}>{isLogin ? 'Đăng nhập ngay' : 'Đăng ký tài khoản'}</button>
            <div style={styles.switchText}>{isLogin ? 'Chưa có tài khoản?' : 'Đã có tài khoản?'}<span style={styles.switchLink} onClick={() => setIsLogin(!isLogin)}>{isLogin ? 'Đăng ký' : 'Đăng nhập'}</span></div>
          </div>
        </div>
      )}
    </header>
